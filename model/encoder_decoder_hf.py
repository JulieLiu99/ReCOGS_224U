        ...
        if labels is not None: # torch.Size([128, 109])
            ...
            ################################# Loss of rows without AND #####################################
                ...
                if AND_row_indices.any():
                    ...
                    diff = torch.cat((torch.zeros((bs, 1), dtype=torch.bool).to(device), AND_in_labels[:, 1:] != AND_in_labels[:, :-1]), dim=1)
                    x = torch.cumsum(diff, dim=1)

                    # y is indices of token within each clause
                    y = torch.zeros_like(AND_in_labels)
                    for i in range(bs):
                        y[i, :] = torch.cat([torch.arange(n).to(device) for n in torch.unique_consecutive(AND_in_labels[i], return_counts=True)[1].tolist()])
                    ...
                    diff = torch.cat((torch.zeros((bs, 1), dtype=torch.bool).to(device), AND_in_pred[:, 1:] != AND_in_pred[:, :-1]), dim=1)
                    x = torch.cumsum(diff, dim=1)

                    y = torch.zeros_like(AND_in_pred)
                    for i in range(bs):
                        y[i, :] = torch.cat([torch.arange(n).to(device) for n in torch.unique_consecutive(AND_in_pred[i], return_counts=True)[1].tolist()])

                    rows = torch.arange(bs).unsqueeze(-1).repeat(1, nl).flatten().to(device)

                    pred_3D = torch.zeros(bs, nt, nl).long().to(device)
                    pred_3D[rows, x.flatten(), y.flatten()] = pred.flatten()

                    logits_4D = torch.zeros(bs, nt, nl, vs).to(device)
